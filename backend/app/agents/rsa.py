...
"""
Resume Strategist (Clean Version)
---------------------------------
This agent is fully compatible with:
- Supervisor Agent
- Terminal UI
- LangGraph (stateless agent)
- Groq Llama 3.3 70B

Outputs:
- resume_json     (structured object)
- resume_inventory (plain text summary)
"""
...
# -------------------------------------------------------------------
# Helper: Fetch GitHub data (parallel)
# -------------------------------------------------------------------
...
def fetch_github_data(username: str) -> Dict[str, Any]:
    """Fetch GitHub profile + repos in parallel."""
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            profile_f = executor.submit(fetch_profile, username)
            repos_f = executor.submit(fetch_repos, username)
            return {
...
Rules:
- Create a strong 3–sentence professional summary.
- Rewrite GitHub repos into professional project descriptions.
- Ensure the JSON is strictly valid. Output ONLY JSON with no commentary.
""")
        ...
        try:
            ...
            return {
                "name": github_data.get("profile", {}).get("name", "Candidate"),
                "location": github_data.get("profile", {}).get("location", "India"),
                "github_username": github_username or "",
                "target_role": target_role,
                "summary": f"Strong aspiring {target_role} with hands-on skills.",
                "skills": user_skills[:20],
                "education": ["B.Tech in Computer Science (In Progress)"],
                "projects": []
            }

    # -------------------------------------------------------------
    def generate_resume_inventory(self, resume_json: dict) -> str:
        """Readable plain-text summary for Curriculum & Supervisor."""
...
Name: {name}
Target Role: {resume_json.get('target_role')}
...
"""
    ...
    def generate_resume(
        ...
        # If GitHub available, fetch GitHub data
        github_data = fetch_github_data(github_username) if github_username else {}

        resume_json = self.generate_resume_json(
            target_role,
            github_username,
            github_data,
            user_skills
        ...
        resume_inventory = self.generate_resume_inventory(resume_json)
